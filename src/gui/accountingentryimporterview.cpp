    ...
    ui(new Ui::AccountingEntryImporterView)
{
    ...
    accountingEntryTable->setColumnCount(7);

    QStringList headerStringList;
    headerStringList << tr("Member") << tr("Beitrag") << tr("Spende") << tr("CCC") << tr("Datum") << tr("Betrag") << tr("Transaction");
    accountingEntryTable->setHorizontalHeaderLabels(headerStringList);

    accountingEntryTable->setItemDelegateForColumn(0, new MemberListDelegate());
